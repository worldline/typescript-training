...
type ToppingOfPizza<P extends Pizza> = P[number];
type ToppingOfPizza<P extends Pizza> = P[0] | P[1] | P[2];
...
// The Pizza store has an infinite supply of cheese and olives
// Make an AvailablePizzas that use these 3 toppings as default value for the Toppings parameter
type InfiniteSupplies = ["cheese", "olives"];
type InfiniteSupply = InfiniteSupplies[number];
type AvailablePizzas<Toppings extends Topping[] = InfiniteSupplies> = Toppings[number] | InfiniteSupply extends infer T 
? [T,T,T] 
: never;
